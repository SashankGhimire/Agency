  ...
  return (
    <section id="about" className="py-24 relative overflow-hidden bg-white dark:bg-[#050505] border-y border-zinc-200/60 dark:border-zinc-800/60">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="p-8 sm:p-10 rounded-3xl bg-zinc-900 text-white dark:bg-[#0a0a0d] border border-zinc-800">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 divide-y sm:divide-y-0 sm:divide-x divide-zinc-800">
            {stats.map((stat, idx) => (
              <div key={stat.label} className={`pt-6 sm:pt-0 ${idx > 0 ? 'sm:pl-8' : ''}`}>
                <span className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight block font-mono">
                  {stat.value}
                </span>
                <span className="text-xs font-bold text-blue-400 mt-1 block uppercase tracking-wider">
                  {stat.label}
                </span>
